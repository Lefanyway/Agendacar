...
const locaisRetirada = [
  {
    value: 'AgendaCar UniFECAF',
    label: 'AgendaCar UniFECAF',
    endereco: 'Campus UniFECAF - unidade principal de retirada',
    horario: 'Segunda a sábado, das 09h às 18h',
  },
  {
    value: 'AgendaCar Paulista',
    label: 'AgendaCar Paulista',
    endereco: 'Unidade central para retirada rápida',
    horario: 'Segunda a sexta, das 08h às 20h',
  },
  {
    value: 'AgendaCar Congonhas',
    label: 'AgendaCar Congonhas',
    endereco: 'Ponto de retirada próximo ao aeroporto',
...
export default function DetalheCarro() {
  ...
  const localSelecionado = locaisRetirada.find((local) => local.value === form.destino)
  ...
  function getImagemCarro(imagem) {
    if (!imagem) return '/img/carro.png'
  ...
  async function handleReserva(e) {
    ...
    if (!form.destino) {
      setMsg({ tipo: 'erro', texto: 'Selecione uma unidade de retirada.' })
      return
    }

    if (!form.dataInicio || !form.dataFim) {
      setMsg({ tipo: 'erro', texto: 'Informe as datas de retirada e devolução.' })
      return
    }
    ...
    try {
      ...
      setMsg({
        tipo: 'erro',
        texto: err.response?.data?.erro || 'Erro ao processar reserva.',
      })
    } finally {
      setEnviando(false)
    }
        ...
        >
          <div className="flex flex-col md:flex-row">
            <div
              ...
              style={{
                background:
                  'radial-gradient(circle at 60% 60%, #3a83f9 0%, #0056e0 100%)',
              }}
            >
              <div
              ...
              <div className="mb-6">
                ...
                <div className="flex flex-wrap gap-3 mt-4">
                  {[
                    ...
                    <span
                      key={label}
                      className="flex items-center gap-1.5 text-sm text-gray-600 bg-gray-100 px-3 py-1.5 rounded-xl"
                    >
              ...
              <form onSubmit={handleReserva} className="space-y-3">
                <div>
                  ...
                  {localSelecionado && (
                    <motion.div
                      ...
                      animate={{ opacity: 1, y: 0 }}
                      className="mt-2 rounded-xl border border-brand-deeper/15 bg-brand-deeper/5 px-3 py-2.5"
                    >
                ...
                {dias > 0 && (
                  ...
                  >
                    ...
                    <span className="font-extrabold text-brand-deeper">
                      {formatBRL(dias * carro.precoDia)}
                    </span>
                  </motion.div>
                )}
